...
class XSVerification{
    ...
    public function init()
    {
        ...
        if(!defined('WWW_PATH')){
            exit("请先引入 config/bootstrap.php");
        }
        $this->ResourcesPath =  'statics/images'.DS.'dragcheck'; //背景图片存放目录 （Resources）
        //$this->ResourcesPath =  APP_PATH.'Common/Resources/Admin/Code/'; //背景图片存放目录 （Resources）
        $dirtmp = 'attachments/code/'.date('YmdH');  //被处理图片 存放目录（裁剪小图，错位背景图）
        $dir = WWW_PATH.$dirtmp;
        $this->tailoring_big_save_path = $dir.'/big/';
        $this->tailoring_small_save_path = $dir.'/small/';
        $this->getRandomPng();
        $tmpspn = md5(uniqid()).$this->picSuffix;
        $this->smallPicName  =  $this->tailoring_small_save_path.$tmpspn;
        $tmpbpn = md5(uniqid().time()).$this->picSuffix;
        $this->bigPicName    =  $this->tailoring_big_save_path.$tmpbpn;

        $this->smsrc = "/".$dirtmp.'/small/'.$tmpspn;
        $this->bigsrc = "/".$dirtmp.'/big/'.$tmpbpn;
    ...
    private function getRandomPng(){
        ...
        $this->srcPic  = WWW_PATH.$this->ResourcesPath.DS.$this->bigPicName.mt_rand(1,4).'.png';
    ...
    public function RestructuringCutting($srcFile,$bigPicName,$thumbnailWide,$thumbnailHeight)
    {
        ...
        $_SESSION[$session_name] = $this->location_x;
        return array(
            'session_name' => $this->session_name,
            'data'    => $_temp_xy_data,
            'bg_pic_src'  => ltrim($this->bigsrc,'.'),
            'bg_pic'  => ltrim($bigPicName,'.'),
            'ico_pic' => array('src'=>$this->smsrc,'url'=> ltrim($this->smallPicName,'.'),'w'=> $this->tailoring_w ,'h'=>$this->tailoring_h),
